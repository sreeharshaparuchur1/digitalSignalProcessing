function [h] = transitionband(N,wc,tbvals)
  % Creating a vector of N equally-spaced frequencies
  w = 0:2*pi/N:2*pi*(N-1)/N;
  % Create ideal amplitude response of low-pass filter (it should be symmetric about w = pi)
  Ad = zeros(size(w));
  for p = 0:N-1
  ...
  %falls exactly on a sample, use that sample and the one to the right).
